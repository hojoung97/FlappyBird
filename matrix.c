...
void draw_bird() {
	int height = (int)sizeof(bird) / sizeof(bird[0]);
	int width = (int)sizeof(bird[0]) / sizeof(bird[0][0]);
	mask_canvas(31, 7, height, width, bird);
	generate_image();
}

void draw_pipe(int index) {
	int height = (int)sizeof(pipes) / sizeof(pipes[0]);
	int width = (int)sizeof(pipes[0]) / sizeof(pipes[0][0]);
	mask_canvas(0, 20, height, width, pipes);
	generate_image();
}
